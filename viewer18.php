...
function get_compact_table_data_updated($connection, $records, $rot47_columns) {
    ...
    foreach ($records as $record) {
        ...
        // FIXED: Use dpia_vereist column instead of has_dpia()
        $dpia_vereist = isset($record['dpia_vereist']) ? $record['dpia_vereist'] : 'Nee';
        $row['dpia_vereist'] = $dpia_vereist; // Will show "Ja" or "Nee"
        
        // Also show if DPIA is registered
...
.compact-dpia-req{width:8% !important;}.compact-dpia-reg{width:8% !important;}.compact-update{width:12% !important;}
...
.compact-id{width:4% !important;}.compact-activity{width:30% !important;}.compact-agreement{width:20% !important;}.compact-processor{width:15% !important;}
.compact-dpia-req{width:8% !important;}.compact-dpia-reg{width:8% !important;}.compact-update{width:15% !important;}.full-details-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:5px;font-size:8pt;}
...
<p><?= t('total_records') ?>: <?= count($compact_data); ?></p><p><?= t('columns_shown') ?>: ID, Activity, Agreement with Third Party, We are Processor, DPIA Required, DPIA Registered, Update Needed</p></div></div>
...
<th class="compact-dpia-req"><?= t('dpia_vereist') ?></th><th class="compact-dpia-reg">DPIA Geregistreerd</th><th class="compact-update"><?= t('update_needed_column') ?></th></tr></thead><tbody>
...
<td class="compact-dpia-req"><?= htmlspecialchars($record['dpia_vereist'] ?? 'Nee'); ?></td><td class="compact-dpia-reg"><?= htmlspecialchars($record['dpia_registered'] ?? 'Nee'); ?></td>
<td class="compact-update"><?php if ($record['update_needed'] == 'Yes'): ?><span style="color:#f44336;font-weight:bold;">⚠️ <?= t('yes') ?></span> (<?= $record['days_since_update'] ?> <?= t('days_ago') ?>)
